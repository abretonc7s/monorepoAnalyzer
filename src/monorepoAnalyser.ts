...
async function getUserChoice(
  ...
  const answer = await new Promise<string>((resolve) => {
    rl.question(
      '\nEnter the number of the analysis to run (or "q" to quit): ',
      resolve,
    );
  });
...
async function runMonorepoAnalysis(): Promise<void> {
  ...
  const analysisOptions: AnalysisOption[] = [
    ...
    },
    {
      name: "Size Analysis",
      description:
        "Analyze built package sizes and compare across repositories",
      action: analyzeSizeLimits,
    },
    {
      name: "GitHub Analysis",
      description:
        "Analyze GitHub statistics (stars, watchers, forks, releases)",
